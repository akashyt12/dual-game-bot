...
def platform_select_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="\U0001F4B0 BDGWIN", callback_data="platform_bdgwin")],
    ])
...
def game_menu_kb_bdgwin():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="\u26A1 30 SEC", callback_data="bdgwin_30s"),
         InlineKeyboardButton(text="\U0001F525 1 MIN", callback_data="bdgwin_1m")],
        [InlineKeyboardButton(text="\U0001F48E 3 MIN", callback_data="bdgwin_3m"),
         InlineKeyboardButton(text="\u2B50 5 MIN", callback_data="bdgwin_5m")],
        [InlineKeyboardButton(text="\U0001F3C6 10 MIN", callback_data="bdgwin_10m")],
        [InlineKeyboardButton(text="\u25C0 BACK", callback_data="back_menu")]
    ])
...
async def handle_callbacks(callback: CallbackQuery):
    ...
    if data.startswith("platform_"):
        ...
        if p == "jai":
            ...
            img_f = "wingo.png"
            text = box("\U0001F4B0 BDGWIN SELECTED",
                "<b>WinGo 30S / 1M / 3M / 5M / 10M</b>\n\n"
                "Type <b>/login</b> to authenticate\n"
                "Then enter balance to start"
            ) + footer()
        else:
    ...
    if data.startswith("bdgwin_"):
        gm = {"bdgwin_30s": "WinGo_30S", "bdgwin_1m": "WinGo_1M", "bdgwin_3m": "WinGo_3M", "bdgwin_5m": "WinGo_5M", "bdgwin_10m": "WinGo_10M"}
        game_code = gm.get(data, "WinGo_30S")
        user_data["game"] = game_code
        update_user(user_id, user_data)
        nm = {"WinGo_30S": "30 SEC", "WinGo_1M": "1 MIN", "WinGo_3M": "3 MIN", "WinGo_5M": "5 MIN", "WinGo_10M": "10 MIN"}
        try:
            await callback.message.delete()
        except Exception:
            pass
        await send_section(callback.message.chat.id, user_id, "main_menu.png",
            box("\u2705 GAME SET", f"<b>BDGWin WinGo {nm.get(game_code,'30S')}</b>") + footer(), reply_markup=main_menu_kb())
        await callback.answer(f"WinGo {nm.get(game_code,'30S')}")
        return
...
async def run_betting_bdgwin(user_id, chat_id, user_data):
    ...
    game_names = {"WinGo_30S": "30 SEC", "WinGo_1M": "1 MIN", "WinGo_3M": "3 MIN", "WinGo_5M": "5 MIN", "WinGo_10M": "10 MIN"}
    game_name = game_names.get(game, "30 SEC")

    msg = await bot.send_message(chat_id, box("\U0001F4B0 BDGWIN", "Logging in...") + footer(), reply_markup=main_menu_kb())
    _profit_messages[user_id] = msg.message_id
    ...
    _active_bots[user_id] = bot_state
    await update_profit_msg(user_id, chat_id, bot_state, "RUNNING", f"BDGWIN {game_name}")
    ...
    while bot_state["running"]:
        try:
            if not admin:
                ud = get_user(user_id)
                if was_premium and not is_premium_active(ud):
                    await stop_and_show_result(chat_id, user_id, bot_state, f"BDGWIN {game_name}")
                    return
                if points_finished(ud):
                    await stop_and_show_result(chat_id, user_id, bot_state, f"BDGWIN {game_name}")
                    return
            ...
            if open_issue:
                try:
                    ...
                    await update_profit_msg(user_id, chat_id, bot_state, "WAITING", f"BDGWIN {game_name}")
                except Exception as e:
                    logger.error(f"BDGWin Bet failed: {e}")
            await asyncio.sleep(1)
        except Exception as e:
            logger.error(f"BDGWin error: {e}")
            await asyncio.sleep(3)

    if user_id in _profit_messages:
        try:
            await bot.edit_message_text(chat_id=chat_id, message_id=_profit_messages[user_id],
                text=format_profit(bot_state, "STOPPED", f"BDGWIN {game_name}") + footer(), reply_markup=main_menu_kb())
        except Exception:
            pass
        await asyncio.sleep(5)
        if user_id in _profit_messages:
